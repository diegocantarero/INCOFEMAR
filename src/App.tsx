...
function buildDefaultDevices(peopleArr: Person[]) {
  ...
  const es = byName("Estela"); if (es) map[es.id] = ["Samsung Tab A9"];
  return map;
}
...
export default function IncofemarComparativa() {
    ...
    try {
      ...
      if (raw) {
        ...
        if (Array.isArray(p.branches) && p.branches.length) {
          const normalized = p.branches.map((b: any) => {
  ...
  useEffect(() => {
    if (!hydrated) return;
    try {
      const snapshot = { people, dark, branches, mobilePlan, devicesByPerson, planByPerson, contactsByPerson };
  ...
  const addPerson = () => {
    const id = makeId();
    const newP: Person = { id, name: `Persona ${people.length + 1}` };
    setPeople((prev) => [...prev, newP]);
    setPlanByPerson((prev) => ({ ...prev, [id]: "Bss S" }));
    setContactsByPerson((prev) => ({ ...prev, [id]: { phone: "", operator: "Claro Prepago" } }));
  };
  ...
  const setSingleDevice = (pid: string, value: "" | DeviceKey) => {
    setDevicesByPerson((prev) => {
      const next = { ...prev };
  ...
  const downloadPricesSVG = () => {
    const rows = [
      ["Concepto", "Actual", "Claro Personal", "Claro Empresarial", "Tigo Empresarial"],
      ["Precio por línea", L(CURRENT_PER_LINE), L(CLARO_PERSONAL_PER_LINE), L(CLARO_EMP_PER_LINE), L(TIGO_PER_LINE)],
      ["Líneas", String(lines), String(lines), String(lines), String(lines)],
      ["Total mensual", L(actualMonthly), L(claroPersMonthly), L(claroEmpMonthly), L(tigoMonthly)],
      ["Ahorro vs Actual", "—", L(ahorroClaroPersonal), L(ahorroClaroEmp), L(ahorroTigo)],
    ];
    const colW = [200, 160, 180, 200, 180];
    const rowH = 28;
    const width = colW.reduce((a, b) => a + b, 0) + 40;
    const height = rowH * rows.length + 40;
    let y = 20;
    let svg = `<svg xmlns='http://www.w3.org/2000/svg' width='${width}' height='${height}' font-family='Inter,system-ui,Arial' font-size='12'>`;
    svg += `<rect x='0' y='0' width='${width}' height='${height}' fill='${dark ? "#111827" : "#ffffff"}'/>`;
    rows.forEach((r, i) => {
      let x = 20;
      r.forEach((cell, ci) => {
        const w = colW[ci];
        const bg = i === 0 ? (dark ? "#1f2937" : "#F3F4F6") : (dark ? "#374151" : "#F9FAFB");
        svg += `<rect x='${x}' y='${y}' width='${w}' height='${rowH - 4}' rx='6' ry='6' fill='${bg}'/>`;
        svg += `<text x='${x + 10}' y='${y + (rowH - 4) / 2}' dominant-baseline='middle' fill='${dark ? "#E5E7EB" : "#111827"}'>${cell}</text>`;
        x += w;
      });
      y += rowH;
    });
    ...
    <div className={`min-h-screen w-full ${bgApp}`}>
      <div className="mx-auto w-full max-w-none p-3 sm:p-4 md:p-6 lg:p-8">
        <header className="mb-6 flex flex-col gap-2 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className={`text-sm ${subtle}`}>
              Actual vs <b>Claro Personal</b> vs <b>Claro Empresarial</b> vs <b>Tigo Empresarial</b> — {lines} líneas, valores en
              Lempiras (ISV incluido). ISV aplicado: <b>15%</b>.
            </p>
          </div>
        ...
        <section className={`${card} mt-6 rounded-2xl p-5 shadow-sm`}>
          ...
          <p className={`mt-2 text-xs ${subtle}`}>Puedes arrastrar y soltar filas para reordenar; editar nombre, teléfono, operador, dispositivo y plan. El ISV se aplica a (plan sin ISV + financiamiento).</p>
        </section>
        ...
        <section className="mt-6 grid gap-4">
          <div className={`${card} rounded-2xl p-4 shadow-sm`}>
            <div className="mb-3 flex items-center justify-between">
              <h2 className={`text-base font-bold ${subtle}`}>Servicios residenciales por ubicación</h2>
              <button
                type="button"
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-6 items-center gap-2 border-t pt-2">
                <div className="md:col-span-5 text-right font-bold">Total residencial</div>
                <div className="text-right font-extrabold">{L(residencialesTotal)}</div>
              </div>
            </div>
          </div>
        </section>
        ...
        <section className={`${card} mt-6 rounded-2xl p-4 shadow-sm`}>
          ...
          <div className="mt-2 space-y-1 text-sm">
            ...
            <div className="flex items-center justify-between">
              <span>Servicios residenciales seleccionados</span>
              <span className="font-semibold">{L(residencialesTotal)}</span>
            </div>
            <div className="flex items-center justify-between">
              <span>Total mensual (móvil + residencial + equipos)</span>
              <span className="font-bold">{L(totalSinEquipo)}</span>
            </div>
            <p className={`mt-2 text-xs ${subtle}`}>ISV Honduras = 15%.</p>
          </div>
        </section>
